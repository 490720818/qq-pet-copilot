...
def write_version_file(version_file: Path, version: str, repo: str) -> None:
    ...
    repo = str(repo or '').strip() or '490720818/qq-pet-copilot'
    content = (
        '"""应用版本信息。\n\n'
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Write src/version.py from tag.')
    parser.add_argument('--tag', default='', help='Release tag, e.g. v1.2.3')
    parser.add_argument('--repo', default='490720818/qq-pet-copilot', help='GitHub repository slug')
    parser.add_argument('--output', default='src/version.py', help='Output python file path')
